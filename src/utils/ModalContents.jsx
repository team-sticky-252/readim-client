  ...
  message: {
    ...
    test: (
      <p>
        ...
        위해, 브라우저 한정 명령어(<code>alert</code> 등)는 최소한으로
        ...
        실행하고 싶다면 <code>node my.js</code>와 같은 명령어를 사용하면 됩니다.
        <h2>script 태그</h2>
        <pre>
          <code>
            &lt;!DOCTYPE HTML&gt;
        ...
        <p>
          <code>&lt;script&gt;</code> 태그엔 자바스크립트 코드가 들어갑니다.
          브라우저는 이 태그를 만나면 안의 코드를 자동으로 처리합니다.
        </p>
        <h2>모던 마크업</h2>
        <p>
          <code>&lt;script&gt;</code> 태그엔 몇 가지 속성(attribute)이 있습니다.
          요즘엔 잘 사용하진 않지만, 오래된 코드에서 종종 이 속성을 발견할 수
          있습니다.
        </p>
        <p>
          <code>type</code> 속성: <code>&lt;script type=…&gt;</code> HTML4에선
          스크립트에 type을 명시하는 것이 필수였습니다. 따라서{" "}
          <code>type=&ldquo;text/javascript&ldquo;</code> 속성이 붙은 스크립트를
          어렵지 않게 찾을 수 있었습니다. 이젠 타입 명시가 필수가 아닙니다.
          게다가 모던 HTML 표준에선 이 속성의 의미가 바뀌었습니다. 이제 이
          속성은 자바스크립트 모듈에 사용할 수 있습니다. 모듈은 심화 내용이기
          때문에 다른 파트에서 다시 이야기하도록 하겠습니다.
        </p>
      </p>
    ),
